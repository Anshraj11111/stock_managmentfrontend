    ...
    >
      ...
      {(listening || wakeWordActive) && (
        <span className="absolute -top-1 -right-1 flex h-4 w-4">
          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${listening ? 'bg-red-400' : 'bg-blue-400'} opacity-75`}></span>
          <span className={`relative inline-flex rounded-full h-4 w-4 ${listening ? 'bg-red-500' : 'bg-blue-500'}`}></span>
        </span>
      )}
    </button>
  );
